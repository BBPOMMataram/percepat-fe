...
interface FormAtkProps {
    listBarang: any[];
    setListBarang: React.Dispatch<React.SetStateAction<any[]>>;
}

export default function FormAtk({ listBarang, setListBarang }: FormAtkProps) {
    ...
    const fetchAtk = async () => {
        setIsLoading(true);
        try {
            const res = await api.get(`${process.env.NEXT_PUBLIC_BACKEND_URL_PERCEPAT}/api/barang-atk/getAll?name=${searchTerm}`);
            setAtkList(res.data || []);
        } catch (error) {
    ...
    const handleAddBarang = () => {
        if (!selectedAtk) {
            alert("Silakan pilih ATK terlebih dahulu");
            return;
        }
        if (jumlah < 1) {
        ...
        const newItem = {
            id: selectedAtk.id,
            nama: selectedAtk.nama,
            kode: selectedAtk.kode || "",
            nup: selectedAtk.nup || "",
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow border">
                <h3 className="text-lg font-semibold mb-4">Form ATK (Alat Tulis Kantor)</h3>
                                ...
                                {atkList.map((item: any) => (
                                    <tr key={item.id} className="border-t hover:bg-gray-50">
                                        ...
                                        <td className="p-2 text-sm">
                                            <button
                                                onClick={() => setSelectedAtk(item)}
                                                className={`px-3 py-1 rounded text-sm ${selectedAtk?.id === item.id
                                                    ? 'bg-green-500 text-white'
                                                    : 'bg-blue-500 text-white hover:bg-blue-600'
                                                    }`}
                                            >
                                                {selectedAtk?.id === item.id ? 'Dipilih' : 'Pilih'}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
                ...
                {selectedAtk && (
                    <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
                        <p className="text-sm font-medium">ATK Dipilih: {selectedAtk.nama}</p>
                ...
                <div className="mt-4 flex gap-4 items-end">
                    ...
                    <button
                        ...
                        disabled={!selectedAtk}
                        className={`px-4 py-2 rounded ${selectedAtk
