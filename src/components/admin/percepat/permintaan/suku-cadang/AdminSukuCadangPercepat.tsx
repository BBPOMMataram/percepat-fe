...
export default function AdminSukuCadangPercepat() {
    ...
    const downloadSpbHandler = (id: number) => {
        api({
            ...
            .then(({ data }) => {
                ...
                link.href = url;
                link.setAttribute('download', `SPB-SukuCadang-${id}.pdf`);
    ...
    const verifPetugasHandler = (id: number) => {
        ...
        api.post(`/api/v1/verif-petugas-suku-cadang/${id}`,
            {
                user,
    ...
    return (
        <>
            <div className="bg-white rounded-2xl shadow px-8 py-4 mb-2 flex flex-col md:flex-row">
                <div className="text-lg font-semibold text-gray-800 uppercase">Suku Cadang</div>
                ...
                <ul className="max-h-96 overflow-y-auto list-decimal list-inside">
                    {
                        listBarangPermintaan.map((item: any, index: number) => (
                            <li key={index} className="my-1 py-1 px-2 w-fit rounded">
                                {item.sukuCadang ? `${item.sukuCadang.name} (Stok: ${item.sukuCadang.stock})` : `Barang tidak ditemukan (ID: ${item.suku_cadang_id || '-'})`}
