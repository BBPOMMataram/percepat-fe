...
export default function TableReagen() {
    const [reagen, setReagen] = useState<any>()
    const [valuePerPage, setValuePerPage] = useState('5')
    const [nameToSearch, setNameToSearch] = useState('')

    const getData = useCallback(async (url = `api/barang/reagen?value_per_page=${valuePerPage}&name=${nameToSearch}&page=${reagen?.current_page}`) => {
        const { data } = await axiosInstance.get(url)

        setReagen(data);
    }, [valuePerPage, reagen, nameToSearch])
    ...
    const handleSearch = (value: string) => {
        ...
        getData(`api/barang/reagen?value_per_page=${valuePerPage}&name=${nameToSearch}&page=${reagen?.current_page}`)
    }
    ...
    return (
        <>
            <h2 className="text-2xl sm:text-3xl xl:text-5xl my-2">Data Reagen</h2>
