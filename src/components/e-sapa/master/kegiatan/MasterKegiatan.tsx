...
export default function MasterKegiatan() {
    ...
    const fetchData = useCallback((url?: string) => {
        setLoading(true);
        // Menggunakan parameter value_per_page dan name sesuai ApiKegiatanController
        let endpoint = url || `http://localhost:8001/api/kegiatan?page=${currentPage}&value_per_page=${perPage}&name=${activeSearch}`;
    ...
    const handleDelete = (id: number) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus item ini?")) {
            api.delete(`http://localhost:8001/api/kegiatan/${id}`)
                .then((res) => {
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        const payload = {
            ...
            mak: formData.mak,
            maksudTugas: formData.maksud_tugas // <-- Diubah menjadi camelCase sesuai ApiKegiatanController
        };

        const request = formData.id 
            ? api.put(`http://localhost:8001/api/kegiatan/${formData.id}`, payload)
            : api.post(`http://localhost:8001/api/kegiatan`, payload);
                                ...
                                <div className="form-container flex flex-col">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="flex flex-col">
                                            <label htmlFor="kegiatan" className="mb-2 text-sm">Nama</label>
                                            <input 
                                                type="text" 
                                                id="kegiatan" 
                                        ...
                                        <div className="flex flex-col">
                                            <label htmlFor="mak" className="mb-2 text-sm">MAK</label>
                                            <input 
                                                type="text" 
                                                id="mak" 
                                                className="bg-primary/5 rounded w-full lg:w-[85%] border-transparent border-b border-b-primary focus:ring-transparent focus:border-transparent focus:border-b-primary focus:outline-none py-2 px-3 text-secondary-content" 
                                    ...
                                    <div className="flex flex-col mt-6">
                                        <label htmlFor="maksud_tugas" className="font-semibold mb-2 text-sm">Maksud Tugas</label>
                                        <textarea 
                                            id="maksud_tugas" 
                                            rows={4} 
    ...
    return (
        <div className="py-12">
            <div className="sm:mx-6 lg:mx-8 p-6 py-10 bg-secondary text-secondary-content rounded overflow-x-auto shadow-sm">
                        ...
                        {loading ? (
                            ...
                            kegiatan.map((item: any, index: number) => (
                                ...
                                >
                                    ...
                                    <td className="font-semibold">{item.kegiatan}</td>
                                    <td>{item.mak || '-'}</td>
                                    <td className="whitespace-pre-wrap">{item.maksud_tugas || '-'}</td>
                ...
                {/* Handling pagination links structure specifically for Laravel API Resources (res.meta.links or res.links) */}
