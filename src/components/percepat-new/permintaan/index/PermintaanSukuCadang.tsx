...
export default function PermintaanSukuCadangPercepat() {
    ...
    const fetchData = (url?: string) => {
        api.get(`${process.env.NEXT_PUBLIC_BACKEND_URL_PERCEPAT}/api/v1/permintaan-suku-cadang?
            per_page=${perPage}
    ...
    const downloadSpbHandler = (id: number) => {
        api({
            ...
            .then(({ data }) => {
                ...
                link.href = url;
                link.setAttribute('download', `SPB-Suku-Cadang-${id}.pdf`);
    ...
    const confirmDeleteHandler = () => {
        ...
        setIsDeleting(true);
        api.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL_PERCEPAT}/api/v1/permintaan-suku-cadang/${selectedDeleteId}`)
            .then(() => {
                                ...
                                >
                                    ...
                                    <td className="px-1 py-3 flex">
                                        ...
                                        {(item.status?.id < 4 && item.peminta?.external_user_id === user?.id) && (
                                            <Link
                                                href={`/percepat-new/permintaan/form?id=${item.id}&type=suku-cadang`}
