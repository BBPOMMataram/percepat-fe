...
export default function FormAtk({ listBarang, setListBarang }: FormProps) {
    ...
    const fetchBarang = async (query: string) => {
        setIsLoading(true);
        try {
            const res = await api.get(`${process.env.NEXT_PUBLIC_BACKEND_URL_PERCEPAT}/api/v1/barang-atk-all?name=${query}`);
    ...
    const handleAddBarang = () => {
        if (!selectedBarang) {
            dispatch(showAlert({ type: "error", message: "Silakan pilih ATK terlebih dahulu", description: "Silakan pilih ATK terlebih dahulu" }));
            return;
        }
        if (jumlah < 1) {
        ...
        dispatch(showAlert({ type: "success", message: "Berhasil menambahkan ATK", description: `Berhasil menambahkan ${selectedBarang.name}` }));
    ...
    const handleRemoveBarang = (index: number) => {
        ...
        dispatch(showAlert({ type: "success", message: "Berhasil menghapus ATK", description: `Berhasil menghapus ${item.nama}` }));
    };
    ...
    return (
        <div className="space-y-6">
            <div className="bg-white p-4 rounded-lg shadow border">
                <h3 className="text-lg font-semibold mb-4">Form ATK</h3>

                {/* Search and Select ATK */}
                <div className="mb-4">
                    ...
                    <input
                        ...
                        placeholder="Cari atk berdasarkan nama..."
                        value={searchTerm}
                ...
                {/* Selected ATK Details */}
                {selectedBarang && (
                    <div className="mt-4 p-3 bg-blue-50 rounded border border-blue-200">
                        <p className="text-sm font-medium">ATK Dipilih: {selectedBarang.name}</p>
            ...
            {listBarang.length > 0 && (
                <div className="bg-white p-4 rounded-lg shadow border">
                    <h3 className="text-lg font-semibold mb-4">Daftar ATK yang Diajukan</h3>
