...
export default function VerifSukuCadangPercepat() {
    ...
    const getData = useCallback(() => {
        api.get(`${process.env.NEXT_PUBLIC_BACKEND_URL_PERCEPAT}/api/v1/verif-suku-cadang?per_page=${perPage}&kode_or_name=${kodeBarangOrNameFilter}&katim_id=${user?.id}&is_kabagtu=${user?.employee?.group_jabatan_id === 2 ? 1 : 0}` // 2 = Kabag TU
        )
            .then(({ data }) => {
                setDataBarang(data)
    ...
    const verifHandler = (id: number) => {
        ...
        api.post(`/api/v1/verif-katim-suku-cadang/${id}`)
            .then(res => {
                console.log(res);
    ...
    const verifKabagTuHandler = (id: number) => {
        ...
        api.post(`/api/v1/verif-kabagtu-suku-cadang/${id}`,
            {
                user
            })
            .then(res => {
    ...
    return (
        <>
            <h2 className="mb-5 font-bold text-lg lg:text-3xl font-serif">Data Permintaan Suku Cadang</h2>
                                ...
                                >
                                    ...
                                    <td className="px-4 py-3 flex gap-1">
                                        ...
                                        </span>
                                        {
                                            ...
                                            <span className="btn btn-sm btn-success tooltip tooltip-success tooltip-left" data-tip="Verif sebagai KaTim" onClick={() => verifHandler(item.id)}>
                                                <span className="material-symbols-outlined">
                                                    check_small
                                                </span>
                                            </span>
                                        ...
                                        {
                                            (item.status_id === 2) &&
                                            <span className="btn btn-sm  btn-error tooltip tooltip-error tooltip-left" data-tip="Verif sebagai KabagTU" onClick={() => verifKabagTuHandler(item.id)}>
                                                <span className="material-symbols-outlined">
                                                    check_small
                                                </span>
                                            </span>
                            ...
                            listBarangPermintaan.map((item: any, index: number) => (
                                <li key={index} className="my-1 py-1 px-2 w-fit rounded">
                                    ...
                                    <div className="text-xs [&>span]:mr-1">
                                        ...
                                        <span className="badge badge-soft badge-primary">Satuan : {item.sukuCadang?.satuan || '-'}</span>
